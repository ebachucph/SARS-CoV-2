...
Use R to calculate the Shannon entropy.
...
Load Omicron variant mutations from [reference]()
```{r variant-mutations}
...
# Save plot
ggsave("./images/publication/figure2.svg", f2, width = width2, height = width1, units = "mm", device = "svg")
...
Calculate pairwise Jaccard similarity between the peptide sets generated from the coronaviridae and HCMV.
...
# Save plot
ggsave("./images/publication/figure3.svg", f3, device = "svg", width = width2, height = width1, units = "mm")

# Clear space
rm(list = c(ls(pattern = "complete_pairwise_distances"), "cormat", "f3"))
```

Calculate Jaccard distances between the response peptide sets found for each organism investigated.
...
V(network.graph)$color <- ifelse(str_detect(names(V(network.graph)), "Pos"),"dodgerblue1", "firebrick2")
...
Seperate the network graph into IgA and IgG
...
network.igg <- network.graph %>% 
  ...
  ggplot() +
    ...
    scale_color_manual(values = c("SARS2 Neg. IgG" = "grey40", "SARS2 Neg. IgA" = "grey70",
                                  "SARS2 Pos. IgG" = "grey40", "SARS2 Pos. IgA" = "grey70")) +
    scale_fill_manual(values = c("SARS2 Neg. IgG" = "firebrick2", "SARS2 Neg. IgA" = "firebrick2",
                                  "SARS2 Pos. IgG" = "dodgerblue1", "SARS2 Pos. IgA" = "dodgerblue1")) +
...
f4 <- ggarrange(plotlist = list(network.igg, network.iga), ncol = 1,
                                  common.legend = T, legend = "right", labels = "AUTO")

ggsave("./images/publication/figure4.svg", f4, device = "svg", width = width2, height = width2, units = "mm")

# Clear space
rm(list = c("f4", ls(pattern = "net|dist|set|edge")))
```
...
## With X
entropy_x <- list.files("./data/bs_entropy_x", full.names = T) %>% 
  map(., ~fread(.x) %>% mutate(gene_abbr = str_replace(basename(.x), "\\.txt", ""))) %>% 
  rbindlist(.) %>% 
  split(., str_detect(.$gene_abbr, "ORF1")) %>% 
  map2(., names(.), ~if(.y) {
...
```{r response-sequence-summary, message=FALSE, warning=FALSE}
